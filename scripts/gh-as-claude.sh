#!/usr/bin/env bash
#
# Run a single `gh` command under the derek-dorazio-agent-claude[bot] App
...
#
# Usage:
#   ./scripts/gh-as-claude.sh pr review 13 --approve --body-file findings.md
#   ./scripts/gh-as-claude.sh api /installation/repositories
#
...
KEY_PATH="$HOME/.config/github-apps/derek-dorazio-agent-claude.private-key.pem"

if [[ ! -f "$KEY_PATH" ]]; then
  echo "Error: claude App private key not found at $KEY_PATH" >&2
