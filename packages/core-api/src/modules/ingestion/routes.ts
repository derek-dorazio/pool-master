...
export async function ingestionModule(
  ...
  await app.register(async (mutatingRoutes) => {
    await mutatingRoutes.register(adminAuth);
    ...
    mutatingRoutes.post<{ Params: { sport: string; eventId: string } }>(
    ...
    mutatingRoutes.post<{ Params: { sport: string } }>('/odds/:sport', {
      schema: {
        tags: ['Admin'],
