...
describe('admin support services', () => {
  ...
  describe('platform config services', () => {
    ...
    it('pool-master-33l.8.10 submits only discovery feeds when no event is selected', async () => {
      const setImmediateSpy = jest
        .spyOn(global, 'setImmediate')
        .mockImplementation(() => undefined as any);
      const createSyncRun = jest.fn(({ data }) => Promise.resolve({
        id: `sync-${data.payloadJson.requestedFeed}`,
      ...
      try {
        const result = await service.runContestQaWorkflow({
          mode: 'PREPARE_CONTEST_EVENT_DATA',
          sport: Sport.GOLF,
        }, 'admin-1', 'admin@example.com');

        expect(result.steps.map((step) => [step.id, step.status])).toEqual([
          ['discover-and-rank-events', 'SUBMITTED'],
          ['hydrate-event-field', 'BLOCKED'],
        ]);
        expect(createSyncRun.mock.calls.map(([arg]) => arg.data.payloadJson.requestedFeed)).toEqual([
          'EVENTSCHEDULE',
          'PARTICIPANTRANKINGS',
        ]);
        expect(result.warnings).toEqual(expect.arrayContaining([
          expect.objectContaining({ code: 'EVENT_SELECTION_REQUIRED' }),
          expect.objectContaining({ code: 'NO_FUTURE_EVENTS_KNOWN' }),
        ]));
      } finally {
    ...
    it('pool-master-33l.8.10 submits event-state and live-score feeds in order for live testing', async () => {
      const setImmediateSpy = jest
        .spyOn(global, 'setImmediate')
        .mockImplementation(() => undefined as any);
      const createSyncRun = jest.fn(({ data }) => Promise.resolve({
          ...
          findMany: jest.fn().mockResolvedValue([
            {
              ...
              releaseAt: new Date('2026-05-14T12:00:00.000Z'),
              fieldLocksAt: new Date('2026-05-27T16:00:00.000Z'),
              fieldLocked: false,
              _count: { sportEventParticipants: 80 },
            },
          ]),
        },
        providerSyncRun: {
      ...
      try {
        ...
        expect(result.steps.map((step) => [step.id, step.status, step.syncRunIds])).toEqual([
          ['apply-event-state', 'SUBMITTED', ['sync-EVENTPARTICIPANTS']],
          ['sync-live-scores', 'SUBMITTED', ['sync-EVENTLIVESCORES']],
        ]);
        expect(createSyncRun.mock.calls.map(([arg]) => arg.data.payloadJson.requestedFeed)).toEqual([
          'EVENTPARTICIPANTS',
          'EVENTLIVESCORES',
        ]);
        expect(createSyncRun.mock.calls.map(([arg]) => arg.data.eventId)).toEqual([
          'golf-relative-weekend-20260528',
          'golf-relative-weekend-20260528',
        ]);
      } finally {
        setImmediateSpy.mockRestore();
