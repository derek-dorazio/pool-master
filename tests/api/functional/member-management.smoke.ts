...
describe('Member Management + Permissions', () => {
  beforeAll(async () => {
    ...
    // Create league
    const lr = await smokeFetch(`${BASE_URL}${API_ROUTES.leagues.list}`, {
    ...
    // Invite + accept
    const il = await smokeFetch(`${BASE_URL}${API_ROUTES.leagues.inviteLink(leagueId)}`, {
      method: 'POST', headers: h(ownerToken),
      body: JSON.stringify({ expiresInDays: 7, maxUses: 5 }),
    });
    const code = ((await il.json()) as any).invitation?.inviteCode;
    await smokeFetch(`${BASE_URL}${API_ROUTES.invitations.accept}`, {
  ...
  it('owner promotes member to COMMISSIONER', async () => {
    const res = await smokeFetch(`${BASE_URL}${API_ROUTES.leagues.memberRole(leagueId, memberId)}`, {
  ...
  it('commissioner can update league settings', async () => {
    const res = await smokeFetch(`${BASE_URL}${API_ROUTES.leagues.settings(leagueId)}`, {
  ...
  it('owner can remove a member', async () => {
    const res = await smokeFetch(`${BASE_URL}${API_ROUTES.leagues.removeMember(leagueId, memberId)}`, {
      method: 'DELETE',
