...
10. **PR review (multi-pass, see below)**: `Riley` Pass 1 (implementer self-check, in PR body marker) → `Riley` Pass 2 (cross-model secondary, via `gh pr review`) → optional specialty passes when the slice triggers them: `Sage` Pass 3 (security), `Archie` Pass 4 (architecture), `Felix` Pass 5 (frontend discipline), and `Perry` Pass 6 (performance).
...
| 5 | `Felix` (frontend discipline check) | When slice touches `clients/poolmaster`, frontend tests, shared UI primitives, or frontend rules | `gh pr review` from a separate App identity |
| 6 | `Perry` (performance check) | When slice touches Prisma queries, route/list payloads, ingestion/scoring hot paths, list rendering, new runtime dependencies, or similar performance surfaces | `gh pr review` from a separate App identity |
...
Each `gh pr review` post (Passes 2-6) starts with the persona+pass+model header per `workflow-rules.md §6`:
...
- Felix is the frontend-discipline reviewer. PRs touching the PoolMaster web
  app get Riley plus Felix so React, SDK/type, state, theme, a11y, and frontend
  test-shape issues have a dedicated lens.
- Perry is the performance reviewer. PRs touching data access, hot paths,
  payload size, bundle shape, list rendering, or new runtime dependencies get a
  dedicated performance lens.
