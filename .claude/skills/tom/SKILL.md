---
name: tom
description: Technical specification persona — DORMANT. Only invoke explicitly with /tom for major new features that need pre-implementation technical framing. Not auto-routed; per ADR-0003 tech specs are skip-by-default. Before acting, Read personas/tom.md for the full playbook.
...
**This persona is dormant.** It does not auto-route. Use only when the user explicitly requests it via `/tom` — for major new features that need pre-implementation technical framing before Brad/Fran begin. Per ADR-0003, tech specs are optional and deleted when the implementation ships.
