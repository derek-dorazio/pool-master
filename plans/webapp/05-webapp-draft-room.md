...
- The shared draft room now consumes real backend room contracts across snake, open-selection, tiered, pick'em, and bracket modes.
- The page no longer redirects users away or silently no-ops when the room is unavailable or the user lacks a contest entry; it now shows explicit truthful states and routes the user back to the contest entry flow.
- The bottom draft chat is now the live contest chat panel backed by `/api/v1/social/contests/:contestId/chat`, rather than a placeholder drawer with a fake send action.
- Budget-pick rooms now preserve real pricing metadata from the backend draft contract, so the room can show live participant prices plus per-entry budget spent/remaining instead of flattening that mode into a generic selection flow.
- Remaining work in this area should continue from real draft-state, matchup, and contest-entry contracts rather than frontend-only room assumptions.
...
| W-DR-008 | 2 | Draft chat panel — collapsible chat with message feed, pick announcements, commissioner action messages, unread badge | Done | The draft room now embeds the live contest chat panel backed by `/api/v1/social/contests/:contestId/chat` instead of a placeholder input. Pick/system message richness can continue evolving on top of the real social contract |
...
| W-DR-011 | 3 | Salary cap / auction UI variant — auction stage, bidding controls, bid clock, nomination flow, auction log, budget tracker | In Progress | The live `BUDGET_PICK` room now carries real `budget`, `pricingMethod`, and per-pick price data through the draft DTO, and the web room surfaces actual participant prices plus spent/remaining budget in the roster/overview panels. Remaining work is the deeper auction-specific nomination/bidding workflow if that contest mode is still intended beyond the current budget-cap roster selection contract |
| W-DR-012 | 3 | Tiered draft UI variant — tier-grouped pick board, tier filters, tier slot indicators on roster panel | Done | Tiered room is now wired to the live draft DTO and persisted roster picks: the page branches on `selectionType`, renders the real tier config from the backend, and submits tiered selections through `/api/v1/drafts/:contestId/pick` instead of pretending every contest is a snake draft |
| W-DR-013 | 3 | Pick'em UI variant — event/game list with pick selectors, confidence points, per-event deadlines | In Progress | The live room now consumes a real matchup-backed `pickEmEvents` DTO and submits/update picks through `/api/v1/drafts/:contestId/pick`, backed by persisted `contest_matchups` + expanded `contest_picks`. `EVENT_FIELD` pool resolution now also materializes real pick'em matchup rows from ingested event metadata/competitor mappings instead of requiring hand-seeded rows. Remaining work is broader multi-event/slate ingestion beyond single-event matchup generation |
| W-DR-014 | 3 | Bracket pick'em UI variant — bracket visualisation, click-to-advance, auto-fill by seed, reset, completion tracking | In Progress | The draft room now consumes a real `bracketMatchups` backend contract backed by `contest_matchups` + `bracket_predictions`, and the bracket panel now uses live click-to-advance, reset, and auto-fill endpoints instead of frontend-only state. `EVENT_FIELD` pool resolution now seeds first-round bracket matchups from the resolved participant field, so the room no longer depends on manually inserted `contest_matchups`. Remaining work is deeper bracket-specific scoring/results/admin surfaces beyond the shared draft-room experience |
| W-DR-015 | 4 | Commissioner controls — pause, resume, extend, undo, skip | In Progress | Snake-draft commissioner controls are now real end to end: the room renders pause, resume, add-clock, undo, and skip only for actual owners/commissioners, and the backend updates live draft session state through `/api/v1/drafts/:contestId/pause`, `/resume`, `/extend`, `/undo`, and `/skip`. Skip is represented honestly as an explicit skipped pick in the draft grid/results instead of a fake participant. Remaining work is deeper commissioner tooling beyond the shared room, if still needed |
...
| W-DR-016 | 3 | Draft recap page — pick grid, team roster cards, draft analysis (best value, reaches, steals), share and export buttons | Done | The recap page now uses truthful mode-aware labels and real matchup context from the live draft DTO: snake contests stay “Draft Results,” while pick'em and bracket rooms render results copy plus matchup/region labels from `pickEmEvents` and `bracketMatchups` instead of flattening everything into generic draft wording |
