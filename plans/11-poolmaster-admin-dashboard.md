...
| 11-004 | 1 | Tenant list view (name, plan, members, contests, status, last active) | Done | Routes + handler + service exist; treat the current tenant data path as contract-aligned, not yet a finished durable persistence story |
| 11-005 | 1 | Tenant detail view (subscription, usage, leagues, activity) | Done | GET /api/v1/admin/tenants/:tenantId returns the expected shape, but persistence completeness still depends on the remaining contract follow-up |
| 11-006 | 1 | Tenant actions (upgrade, downgrade, apply credit, suspend) | Done | Mutating endpoints are present and audit-logged, but the plan should not imply billing/state side effects are fully durable everywhere |
| 11-007 | 1 | `admin_audit_log` table + all admin actions logged | Done | Audit query/export endpoints and the admin-audit hook exist; avoid reading this as proof that every admin write path is backed by final persistence |
| 11-008 | 2 | User search across tenants (email, name, ID) | Done | GET /api/v1/admin/users exists with contract-backed search params; the result set should still be described as implementation-complete only at the API boundary |
| 11-009 | 2 | User detail view (tenants, leagues, contests, devices) | Done | GET /api/v1/admin/users/:userId returns the dashboard shape, but persistence completeness remains a follow-up concern |
| 11-010 | 2 | User actions (reset password, force logout, disable) | Done | Admin user actions are wired and audited, but the underlying account-state durability is still part of the honest-contract follow-up |
| 11-011 | 2 | Contest browser (any contest, any league, any tenant) | Done | Contest browsing and detail endpoints exist; avoid describing the data as fully authoritative across every persistence layer |
| 11-012 | 2 | Contest admin actions (force close, reopen, recalculate) | Done | Contest mutators exist and are audited, but recalculation/override flows remain contract-aligned until the persistence contract is fully closed |
| 11-013 | 2 | User merge tool (merge duplicates, transfer memberships) | Done | Merge endpoint returns transfer counts and audit events, but downstream reconciliation should not be implied as fully durable everywhere |
| 11-014 | 3 | Provider health dashboard (status, error rate, latency, last event) | Done | GET /providers/health returns provider summaries for operational visibility; do not read this as proof of complete provider-state persistence |
| 11-015 | 3 | Ingestion monitoring view (jobs, errors, throughput) | Done | GET /providers/ingestion returns monitoring data, but the jobs/error history should stay labeled as contract-backed rather than final persistence |
| 11-016 | 3 | Provider configuration management (credentials, webhooks) | Done | GET/PUT provider config endpoints exist and are audit-logged, but credential/webhook persistence still needs the contract hardening follow-up |
| 11-017 | 3 | Re-ingest tools (trigger re-ingestion for specific event) | Done | Operational trigger endpoints exist; they should not be described as guaranteeing end-to-end durable ingestion recovery |
| 11-018 | 3 | `feature_flags` table + CRUD | Done | FlagService uses in-memory Map storage, so this row must not imply durable persistence yet |
| 11-019 | 3 | `feature_flag_overrides` table + tenant-specific overrides | Done | Override logic exists, but storage is still in-memory/contract-level and needs persistence follow-up |
| 11-020 | 3 | Feature flag resolution service (override → global → rollout %) | Done | Resolution logic is implemented, but the backing persistence story is still intentionally incomplete |
| 11-021 | 4 | Service health dashboard (all services, uptime, error rates, latency) | Done | GET /health/services returns service summaries for operational visibility; keep the wording away from implying broader persistence guarantees |
| 11-022 | 4 | Infrastructure metrics display (DB, Redis, queues, S3/CDN) | Done | GET /health/infrastructure and /health/metrics exist, but they are read models and not evidence that every upstream state change is durably stored |
| 11-023 | 4 | Alert configuration (thresholds, channels — Slack, PagerDuty) | Done | Alert config endpoints exist with mute/unmute support, but the doc should avoid implying the alert store is fully production-hardened |
| 11-024 | 4 | Error log viewer (recent errors, stack traces, request IDs) | Done | Error log filters/detail endpoints exist; this is operational visibility, not a blanket claim of complete historical retention |
...
| 11-030 | 5 | Tenant data export tools | Done | ExportService with start/status/download flow; note that the current completion behavior is still contract-shaped and should not be described as a finished durable export pipeline |
