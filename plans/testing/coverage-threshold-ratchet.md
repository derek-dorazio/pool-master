...
| CTR-005 | CI Visibility | Upload coverage artifacts and print actual totals to logs in CI | In Progress | `.github/workflows/ci.yml` now uploads backend, web, and admin coverage artifacts and writes markdown tables to step summaries while printing the same totals to logs. Remaining work is to exercise the next CI run and confirm the artifacts are visible in GitHub UI. |
