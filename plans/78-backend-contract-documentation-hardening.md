...
| 78-007 | 3 | Add a durable backend review checklist or helper guidance for future contract-documentation completeness | Done | Added a durable contract-documentation checklist to `rules/service-rules.md`, wired the checklist into the slice-completion workflow in `rules/workflow-rules.md`, and updated the backend developer persona so future backend/shared API work must treat documentation completeness as part of done rather than follow-up polish. |
| 78-008 | 3 | Decide whether a human-readable Swagger/Redoc publication step is worth adding after the contract source quality is improved | Done | Swagger UI is now exposed on the public app domain at `/apidoc`. Fastify serves the UI at `/apidoc`, and CloudFront forwards `/apidoc*` to the API origin after Terraform apply. |
