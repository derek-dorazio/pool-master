...
| WAH-006 | DTO Alignment | Eliminate remaining local API-facing extensions/casts like [use-profile.ts](../clients/web/src/features/settings/hooks/use-profile.ts) where the backend contract should own the shape | Partial | Removed the local cast in [use-migrations-api.ts](../clients/admin/src/hooks/use-migrations-api.ts) by validating against shared DTO schemas, replaced the fake standings hook shape with shared standings DTO parsing plus new shared scoring DTOs for real score timeline responses, and added real shared DTO-backed compliance/social contracts for consent history, data-export status, conversations, direct messages, contest chat, share cards, and league recaps. The admin cleanup lane also now uses real announcement/template DTO mappings and backend mutations instead of casted shapes or no-op UI behavior; broader cleanup still remains in other hooks and any remaining manual client assumptions |
| WAH-007 | Smoke Tests | Replace the old permissive/seed-dependent smoke layer with MVP-safe deployed black-box flows | In Progress | Legacy API smoke suites were deleted and replaced with [mvp-baseline.smoke.ts](../tests/api/functional/mvp-baseline.smoke.ts) and [contest-lifecycle.smoke.ts](../tests/api/functional/contest-lifecycle.smoke.ts). Remaining work is live QA hardening plus a truthful standings/results smoke once the product exposes a self-owned deployed path |
| WAH-008 | Browser Tests | Keep the required deploy-gate browser suite minimal and stable; defer richer browser journeys to a later non-blocking rebuild | In Progress | The required Playwright lane is now intentionally small and limited to stable public/auth sanity checks in [mvp-browser.smoke.ts](../clients/web/e2e/mvp-browser.smoke.ts). Higher-value invite/contest/participant browser journeys are deferred to [browser-e2e-high-value.md](./testing/browser-e2e-high-value.md) until the codebase and QA data setup are steadier |
