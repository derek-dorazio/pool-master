...
| WAH-006 | DTO Alignment | Eliminate remaining local API-facing extensions/casts like [use-profile.ts](/Users/DDorazio/Library/CloudStorage/OneDrive-CURRICULUMASSOCIATESLLC/Documents/Claude/pool-master/clients/web/src/features/settings/hooks/use-profile.ts) where the backend contract should own the shape | In Progress | Removed the local cast in [use-migrations-api.ts](/Users/DDorazio/Library/CloudStorage/OneDrive-CURRICULUMASSOCIATESLLC/Documents/Claude/pool-master/clients/admin/src/hooks/use-migrations-api.ts) by validating against shared DTO schemas, and replaced the fake standings hook shape with shared standings DTO parsing plus new shared scoring DTOs for real score timeline responses; broader cleanup still remains in other hooks |
