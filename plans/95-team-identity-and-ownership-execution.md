...
| 95-005 | 2 | Frontend developer: implement join-flow team creation and editing | Not Started | Normal new-member join should support team name and icon selection. Co-owner invite acceptance should land in the same register/join flow but show the target team read-only with a welcome message instead of prompting for a new team identity. |
...
| 95-009 | 3 | Backend developer: implement co-owner invite flow from email with existing-user shortcut | In Progress | Added backend-first owner-invitation infrastructure: new `SquadOwnerInvitation` model, preview/accept APIs, league-scoped invite listing and revoke APIs, co-owner invite-by-email flow, immediate provisioning for existing PoolMaster users outside the league, rejection of current league members, and replace-owner guardrails. Remaining work is join/register branch integration and later resend/reminder behavior if we decide to add it. |
| 95-010 | 3 | Frontend developer: add league-scoped `Teams` view with joined teams and pending owner invites | Not Started | All members can browse teams read-only. Commissioners can open/edit any team. Members can edit only their own team. Pending owner invites should be visible/manageable here instead of hidden in a separate admin-only surface. Team row actions should support edit, add co-owner, remove owner, replace owner, and inactivate team. |
