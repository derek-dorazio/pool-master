...
| 95-003 | 1 | Backend developer: align squad/team model and API contract to approved product semantics | Done | Commissioner league creation and member invitation acceptance now provision a default Team instead of deferring Team creation until contest entry. The alignment slice removed stale `iconUrl`, added first-class `iconKey` with the shared built-in Team icon catalog, renamed owner-management semantics away from co-manager language, added commissioner-safe team editing, and added first-class Team inactivation that preserves Team history while removing the Team’s active owners from the league. Users who no longer belong to any active leagues are now inactivated instead of being auto-provisioned a replacement Team. |
| 95-004 | 1 | Backend developer: add backend validation and tests for team join/manage/owner flows | In Progress | Unit coverage is green for Team provisioning, icon-aware Team CRUD, commissioner-targeted team editing, contest-entry fallback removal, Team inactivation member-removal semantics, last-owner Team inactivation semantics, and the new team-owner invitation rules. Contract verification was extended with a Team lifecycle route check, but the local integration runner remains environment-dependent on a reachable PostgreSQL test database. |
...
| 95-010 | 3 | Frontend developer: add league-scoped `Teams` view with joined teams and pending owner invites | Done | Added the first real `Teams` directory route and league-home entry point. The page lists joined teams league-wide, shows pending owner invitations with revoke actions, and lets commissioners jump into any team through the shared team-management page while members only get direct management links for their own team. `Edit Team`, `Add Co-Owner`, `Remove Owner`, `Replace Owner`, and `Inactivate Team` now land through the shared team-management page. |
