...
| 95-005 | 2 | Frontend developer: implement join-flow team creation and editing | In Progress | The co-owner branch is now wired into a dedicated `team-invite` acceptance page with auth-home invitation awareness, read-only team identity, and authenticated acceptance into `My Team`. Remaining work is the normal new-member join step where team name and icon should be editable during invite acceptance. |
