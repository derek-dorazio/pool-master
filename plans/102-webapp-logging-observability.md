...
1. **`sessionId`** — already backend-owned, echoed back from `/api/v1/auth/me`. We don't re-derive it on the client; we read it off the authenticated user object in the session store and include it in every log payload when present.
