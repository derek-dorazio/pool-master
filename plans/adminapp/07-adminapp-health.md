...
|---|---|---|---|---|
| AH-001 | 1 | Build Health Dashboard page layout with three sections (Services, Infrastructure, Key Metrics) and 30s polling infrastructure | Not Started | |
| AH-002 | 1 | Build ServiceTable and ServiceRow components with status dots, metrics columns, and expandable detail | Not Started | |
| AH-003 | 1 | Build InfraCard components for PostgreSQL, Redis, Message Bus, S3/CDN with key metrics and status indicators | Not Started | |
| AH-004 | 1 | Build MetricStatCard components for Active Users, API Requests, Notifications, Active Contests, Live Drafts | Not Started | |
| AH-005 | 1 | Build DependencyList and HealthStatusDot shared components; wire up polling with TanStack Query refetchInterval | Not Started | |
| AH-006 | 2 | Build Error Log page with ErrorLogTable, ErrorRow, ErrorFilters (service, type, severity, date range, tenant, search) | Not Started | |
| AH-007 | 2 | Build ErrorDetail expandable panel and StackTraceViewer with syntax highlighting, line numbers, and copy-to-clipboard | Not Started | |
| AH-008 | 3 | Build AlertRulesTable and AlertRow with SeverityBadge, ChannelSelector icons, and Active/Muted status display | Not Started | |
| AH-009 | 3 | Build AlertEditDialog (threshold, window, channels) and MuteDialog (duration selector, reason) with audit logging | Not Started | |
