...
| **Teams and Owners** | all | `/league/:leagueCode/teams` | Table: Team Name / Team Code (link to Team Home) / Icon / Owners (active owners + pending invite rows). Read-only — actions happen on Team Home. |
...
| Joined Teams list (current: full member list per team) | 342–410 | **Teams and Owners** — BUT reduced to spec columns: Name / Team Code (link) / Icon / Owners. *MISMATCH §5: current tile expands each row with full active-member list; new spec hides members — users see the full team only by clicking through to Team Home* |
