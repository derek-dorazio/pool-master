...
**Beads epic:** `pool-master-784` — see `bd show pool-master-784` for live slice state, child stories, and status. This plan is the narrative companion; task tracking lives in Beads.
...
| Team Home | `/league/:leagueCode/teams/:teamId` | `useTeamAuthority` resolves the Team page from backend-emitted Team `teamRelationship` (`leagueMember`, `owner`, `commissioner`) plus separate global `isRootAdmin`. Team-scoped access checks derive from `teamRelationship` plus `isRootAdmin`; Team does not need a separate descriptive `memberType` field in the first pass. The current UI still resolves those signals into the familiar `owner \| commissioner \| viewer` presentation paths. Root-admin-only delete section remains additionally gated by `useIsRootAdmin()` + inactive status. |
...
`/users/:userId` is a single canonical page with authority-gated action modals, **scoped to the user account only**. Backend enforces the authority rules; frontend reads `viewerAuthority` (or equivalent capability signal) from the response and renders the matching modal set. The backend signal remains a set, but the current UI resolves it into one truthful presentation path: self views use the self-service modal set, root-admin views over another user show the root-admin account controls, and viewer is read-only.
...
- **G1. `useUserAuthority(userId)` (or equivalent authority read)** — reads backend-emitted `viewerAuthority` signal from the user detail response. Signal shape: `{ self, rootAdmin, viewer }` — no commissioner tier on the User page. Frontend does not compute authority client-side; it displays the modal set that matches the backend authority signal. Multiple flags can be true simultaneously in the contract, but the current page still resolves them into the truthful presentation paths above (self-service for self, root-admin controls for non-self root-admin views, viewer fallback otherwise).
...
- **H. `useTeamAuthority(teamId)` hook** — frontend helper over the backend-emitted Team contract. Reads Team `teamRelationship` (`leagueMember`, `owner`, `commissioner`) plus separate global `isRootAdmin` and resolves them into the familiar `owner \| commissioner \| viewer` presentation paths. Team intentionally does not add a separate descriptive `memberType` field in the first pass because ownership is already expressed by `teamRelationship.owner`. Replaces scattered inline `leagueQuery.data?.role === 'COMMISSIONER'` checks.
- **I. `useLeagueAuthority(leagueCode)` hook** — frontend helper over the backend-emitted League contract. Reads `leagueRelationship` (`leagueMember`, `commissioner`) plus separate global `isRootAdmin` and resolves them into the familiar `commissioner \| member` presentation paths. Also usable for commissioner-gated nav items (Create Contest, Manage Contests visibility).
...
- ~~**`ContestConfiguration` mutable vs immutable split**~~ — **RESOLVED** in `pool-master-784.6`: contest detail now emits a typed `contestConfiguration` payload instead of an untyped blob, so Manage Contest and My Entries can reason about mutable fields and entry-cap behavior from generated types instead of `Record<string, unknown>`.
- ~~**Per-contest entry limits**~~ — **RESOLVED** in `pool-master-784.6`: contest detail exposes `contestConfiguration.maxEntriesPerSquad`, My Entries and League Home disable Create Entry client-side when the current Team reaches the cap, and backend entry creation still enforces the limit.
- ~~**Entry creation route**~~ — **RESOLVED** in `pool-master-784.6`: `POST /api/v1/contests/:contestId/entries/me` is create-only and returns `201` for a newly created entry. It never returns an existing entry; callers must use `GET /entries`, `GET /entries/me`, or `GET /:contestId` to inspect current entry state before or after creation.
- **League relationship contract** — no longer an open product question. League surfaces should follow the dedicated League relationship plan in [110-league-relationship-contract-pattern.md](./110-league-relationship-contract-pattern.md): add required `leagueRelationship` (`leagueMember`, `commissioner`) plus separate global `isRootAdmin`. `useLeagueAuthority` should consume that backend-emitted contract rather than infer authority locally from mixed league membership and global auth signals.
- **Team relationship contract** — no longer an open product question. Team surfaces should follow the dedicated Team relationship plan in [109-team-relationship-contract-pattern.md](./109-team-relationship-contract-pattern.md): add required Team `teamRelationship` (`leagueMember`, `owner`, `commissioner`) plus separate global `isRootAdmin`. `useTeamAuthority` should consume that backend-emitted contract rather than infer authority locally from mixed league/team signals. Team does not need a separate descriptive `memberType` field in the first pass.
...
Task state for this plan lives in Beads under epic `pool-master-784`. Use `bd show pool-master-784` for the live slice list, statuses, dependencies, and closeout notes. Do not maintain a parallel task table in this file (see `rules/workflow-rules.md §1 Plans and the Beads Tracker` and `docs/adr/0002-plans-as-narrative-delete-after-epic-closes.md`).
