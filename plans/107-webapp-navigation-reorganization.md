...
| 107-006 | 3 | Add root-admin account-scope action modals to `/users/:userId` | Done | Added backend `viewerAuthority`, admin reset-password, and admin delete-account contracts plus the root-admin account-controls experience when a root admin views another user's canonical User page. Existing self views continue to use the self-service modal set on the same route. Commissioner promote/demote/remove-owner actions remain out of scope here and were moved to row 107-012. Validated with the full required repo gate set before closeout. |
| 107-012 | 3 | Add commissioner per-owner row action menu on Teams and Owners and Team Home | Done | Added a shared owner-action menu component used from both Teams and Owners and Team Home for Remove Owner, Promote to Commissioner, and Demote to Member. Owner rows now link to canonical User pages, surface league role badges from the league-members list, and reuse the same confirmation dialog flow across both surfaces. Backend owner removal now rejects removing the last active owner with `SQUAD_OWNER_REMOVE_REQUIRES_MULTIPLE_OWNERS`, and the UI redirects single-owner removal attempts toward Team Home team inactivation instead. Validated with the full required repo gate set before closeout. |
