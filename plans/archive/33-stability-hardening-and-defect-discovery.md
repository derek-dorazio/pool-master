...
> Archived on 2026-04-04 after the stability hardening wave completed. Keep this file only as historical context; do not treat it as active standing policy.
...
Detailed worker-safe execution slices for the first stability wave live in [plans/archive/2026-04-completed-wave/34-worker-sliced-stability-execution.md](/Users/DDorazio/Library/CloudStorage/OneDrive-CURRICULUMASSOCIATESLLC/Documents/Claude/pool-master/plans/archive/2026-04-completed-wave/34-worker-sliced-stability-execution.md). The remaining active coverage work from that wave ended in [plans/archive/2026-04-completed-wave/35-active-coverage-execution.md](/Users/DDorazio/Library/CloudStorage/OneDrive-CURRICULUMASSOCIATESLLC/Documents/Claude/pool-master/plans/archive/2026-04-completed-wave/35-active-coverage-execution.md).
...
|---|---|---|---|---|
| SHD-001 | Platform | Finish remaining Plan 30 priority items that directly improve deploy determinism and maintenance safety | Done | The remaining priority platform pass removed the last Node 20 GitHub Actions deprecation path (`actions/upload-artifact@v4`), aligned manual deploys to immutable task-definition rollouts, removed mutable `latest` assumptions from Terraform bootstrap defaults, and documented the remote-state-only Terraform workflow. Remaining Plan 30 rows are now normal follow-on platform work rather than unfinished stability hardening. |
| SHD-002 | Architecture | Remove Redis from Terraform, ECS config, docs, and runtime assumptions if it is no longer part of the active architecture | Done | Final cleanup removed Redis from CI test services, local docker compose/scripts, Terraform, admin health source contracts, generated OpenAPI/client artifacts, architecture rules, and active docs. Remaining Redis mentions are historical/archive context or explicitly describe the absence of Redis in the current runtime. |
| SHD-003 | Test Strategy | Use this plan item only as transitional guidance while the fast local suites catch up | Done | Transitional guidance served its purpose during the stability phase. Long-lived suite expectations now belong only in `rules/testing-rules.md`; this plan item should not continue as standing policy. |
...
| SHD-006 | Defect Discovery | Increase coverage in backend Jest, web Vitest, admin Vitest, and DB-backed integration by targeting brittle or under-tested MVP areas | Done | The fast-suite execution wave recorded in [plans/archive/2026-04-completed-wave/35-active-coverage-execution.md](/Users/DDorazio/Library/CloudStorage/OneDrive-CURRICULUMASSOCIATESLLC/Documents/Claude/pool-master/plans/archive/2026-04-completed-wave/35-active-coverage-execution.md) is complete: contest review/results, invite permissions, settings/profile, audit/announcements, dashboard/social, league feed/history/recap, settings compliance/data export, and admin provider/tenant detail all landed with real defect fixes and justified DB-backed integration where needed. Future coverage work should start from a new plan rather than leaving this umbrella item open indefinitely. |
