...
| BSH-010 | Verification | Add backend integration coverage for persistent social, real billing adapters, admin migration runs, and supported MVP validation edges | In Progress | The old DB-heavy integration layer was reset and rebuilt into self-contained CRUD/read/negative suites under [tests/integration/core-api](/Users/DDorazio/Library/CloudStorage/OneDrive-CURRICULUMASSOCIATESLLC/Documents/Claude/pool-master/tests/integration/core-api), including contest, league, invitation/member, contest entry, draft room, standings/results, history/feed/dashboard, permissions, and contest validation coverage. Latest slice adds explicit league dashboard/member/audit permission-edge checks for commissioners vs regular members. Remaining work is focused on persistent social, real billing adapters, and admin migration execution once those production paths are made real |
