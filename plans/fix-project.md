...
| FIX-011 | 4 | Migrate remaining `clients/web` API consumers off manual `api-client.ts` usage | In Progress | Migrated 16 page components and most hooks to hey-api SDK. Some web hooks still need migration (agent was interrupted). |
...
| FIX-014 | 5 | Remove banned mock data from remaining web application files | In Progress | Web hooks migration was interrupted — some files may still need mock data removal. |
| FIX-015 | 5 | Replace local frontend API interfaces with shared DTO imports | In Progress | Admin hooks still have local interfaces (marked with TODOs). Web hooks partially migrated. |
