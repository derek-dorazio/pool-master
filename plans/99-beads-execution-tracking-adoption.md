...
- What is the exact Beads install and initialization workflow on this machine?
...
|---|---|---|---|---|
| 99-001 | 1 | Confirm Beads install/init workflow on the local machine after the repo move | Not Started | Do this in the new session after the machine reboot. |
| 99-002 | 1 | Decide where Beads state should live for this repo | Not Started | Prefer a setup that survives normal local workflow without confusing repo state. |
| 99-003 | 2 | Define the PoolMaster Beads mapping from plan lane -> epic and plan row -> bead | Not Started | Start from the contest lane as the first pilot. |
| 99-004 | 2 | Pilot Beads on the current contest execution lane | Not Started | Keep `plans/` as context while Beads owns live execution state. |
