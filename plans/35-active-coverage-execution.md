...
|---|---|---|---|---|
| ACE-001 | A | Expand contest entry/results/standings/scoring review coverage | Done | Salvaged the stalled contest-review worker slice on the main thread: added join/leave invalidation coverage to `detail.test.tsx`, generic-mode label coverage to `results.test.tsx` and `scoring.test.tsx`, and standings sort coverage to `standings.test.tsx` |
| ACE-002 | B | Expand backend invite and membership permission coverage | Done | Salvaged the stalled backend invite/permissions worker slice on the main thread: added single-use invitation acceptance coverage in `invitation-service.test.ts` and negative-path coverage for missing identity and missing league id in `league-permissions.test.ts` |
| ACE-003 | C | Expand web settings/profile/account-management coverage | Done | The worker slice in `79fddf7` is already part of mainline history: focused Vitest coverage now exercises profile edits, password changes, linked-account loading/connect/disconnect behavior, and keeps the slice constrained to active account-management surfaces. |
| ACE-004 | D | Expand admin audit and announcements coverage | Done | Salvaged the stalled admin audit/announcements worker slice on the main thread: added stable selectors/labels to audit and announcement pages and added focused admin Vitest coverage for audit row expansion, pagination, announcement toggles, and announcement creation |
| ACE-005 | E | Expand backend dashboard/social edge coverage | Done | The slice now includes dashboard-service ordering coverage, social communication ordering/error-path coverage, and DB-backed social feed read coverage that exercises pagination, replies, and pin/unpin behavior through the live route boundary. |
| ACE-006 | Queue | League feed/history/recap coverage | Done | Separated the league feed/history/recap MSW rewrite from the mixed worker commit, added stable selectors, and strengthened the feed tests to prove commissioner-only controls for owner vs manager roles |
...
| ACE-008 | Queue | Admin provider/tenant detail coverage | Done | Separated the provider/tenant detail error-state fixes and coverage from the mixed worker commit; kept the UI/test improvements and left coverage-threshold ratchets on the main thread |
