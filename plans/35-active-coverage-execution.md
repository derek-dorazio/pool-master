...
| ACE-005 | E | Expand backend dashboard/social edge coverage | In Progress | Most of the stalled slice is validated and ready: `dashboard-service.ts` now preserves repository ordering inputs, and unit coverage was added for dashboard sorting/event limits and social conversation ordering/error paths. The new social-feed integration addition still fails on the pin-route assertion and needs follow-up before this slice is marked done. |
