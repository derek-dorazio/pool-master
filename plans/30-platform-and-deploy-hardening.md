...
| PDH-012 | Runtime Drift | Remove unused Redis infrastructure and environment wiring from Terraform and service runtime | In Progress | Redis client usage does not appear in live app code. The current cleanup slice removes Redis from CI test services, local docker compose, Terraform, OpenAPI export env setup, and the admin health DTO/service/UI contract. Remaining work is to refresh generated artifacts, remove the unused `ioredis` dependency, and clean the leftover docs/plan references that still describe Redis as active infrastructure. |
