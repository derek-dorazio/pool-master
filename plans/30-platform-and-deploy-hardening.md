...
| PDH-010 | Test Infra | Replace the legacy DB-backed integration suite with small self-contained CRUD-style integration batches | In Progress | On 2026-04-03/04 the old `tests/integration/core-api` suites were deemed low-value because they mixed stale contracts, seed assumptions, and coupled state across many modules. Reset the layer and rebuild incrementally: start with a single contest CRUD suite that creates its own user/league/contest, uses shared enums/constants, updates via real routes, deletes via real routes, and verifies cleanup. Future agents should continue rebuilding one domain batch at a time rather than restoring the deleted suites verbatim |
