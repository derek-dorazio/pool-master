...
| PDH-012 | Runtime Drift | Remove unused Redis infrastructure and environment wiring from Terraform and service runtime | Pending | Redis is no longer part of the active architecture, but `REDIS_URL` and Redis infrastructure are still present in [main.tf](/Users/DDorazio/Library/CloudStorage/OneDrive-CURRICULUMASSOCIATESLLC/Documents/Claude/pool-master/infrastructure/terraform/main.tf) and common ECS environment wiring. Once QA rollout is stable, remove the dead Redis resources/config and verify no runtime or health paths still claim Redis as a dependency. |
...
| PDH-011 | Coverage Gates | Enforce current coverage baselines locally and in GitHub CI, then ratchet upward over time | Not Started | Add failing thresholds to Jest and both Vitest configs, surface coverage artifacts/log summaries in CI, and protect against regressions by setting thresholds at the current measured baselines first |
