...
|---|---|---|---|---|
| PDH-001 | CI/CD | Remove or sharply reduce `continue-on-error` from deploy-critical steps in [ci.yml](../.github/workflows/ci.yml) | Done | Deploy-critical backend and frontend steps now fail the workflow directly instead of degrading into best-effort drift, and lint/typecheck no longer swallows ESLint failures. |
| PDH-002 | Deploy Strategy | Stop deploying ECS from `:latest` tags referenced in [ci.yml](../.github/workflows/ci.yml) and [main.tf](../infrastructure/terraform/main.tf) | Done | CI now publishes immutable SHA-tagged images, QA rollout uses digest-pinned task definitions, manual deploy promotion expects an immutable tag, Terraform bootstraps from an explicit `core_api_bootstrap_image_tag`, and ECR repositories are configured as immutable. |
| PDH-003 | Deploy Strategy | Move away from “force new deployment against latest image” and register new ECS task definitions per release | Done | QA rollout already registers and deploys explicit task-definition revisions, and the manual environment deploy workflow now registers target-environment core-api and migrate task definitions from the promoted immutable image before updating the ECS service. |
| PDH-004 | QA Jobs | Promote migrate and seed into first-class deploy primitives | Done | `migrate-qa` now treats migration, seed, rollout target capture, stabilization, and downstream smoke/E2E gating as distinct ECS-backed steps, with dedicated migrate logging and rollout verification separated from the schema task itself. |
| PDH-005 | Verification | Add explicit post-deploy verification before smoke | Done | QA rollout now captures expected and PRIMARY task definitions, waits for stabilization, and dumps ECS service events, task metadata, CloudWatch log stream context, and ALB target health before failing. Smoke and Playwright remain gated behind successful stabilization. |
...
| PDH-013 | Frontend Deploys | Make QA web/admin frontend deployments immutable and traceable instead of overwrite-only S3 root syncs | Done | QA web and admin builds publish to immutable `releases/<git-sha>/` prefixes, promote only the root `index.html`, hard-fail on deploy errors, and emit the deployed release prefixes in the CI summary so manual QA can confirm what is live. |
| PDH-006 | Terraform Hygiene | Remove local Terraform artifacts from version control, especially [infrastructure/terraform/.terraform/terraform.tfstate](../infrastructure/terraform/.terraform/terraform.tfstate) | Done | Local `.terraform/terraform.tfstate` is no longer tracked, `.terraform.lock.hcl` remains the only tracked Terraform artifact in that area, and `infrastructure/terraform/README.md` now documents the remote-state-only workflow for shared environments. |
...
| PDH-009 | Node Actions | Upgrade GitHub Actions that still rely on Node 20 | Done | Workflow actions were upgraded earlier, and the remaining `actions/upload-artifact@v4` calls have now moved to `@v5`, removing the last known Node 20 deprecation path in CI. |
| PDH-010 | Test Infra | Replace the legacy DB-backed integration suite with small self-contained CRUD-style integration batches | Done | On 2026-04-03 the old `tests/integration/core-api` suites were removed and replaced with new self-contained CRUD/read-flow suites for contests, leagues, invitations/members, contest entries, draft session flow, and standings/results reads. See [integration-crud-rebuild.md](./testing/integration-crud-rebuild.md) for the completed baseline and [integration-expansion-by-domain.md](./testing/integration-expansion-by-domain.md) for the next worker-split expansion phase. |
...
| PDH-014 | Migration Resilience | Add post-migrate health verification and dead-gate detection to the QA migration lane | In Progress | After repeated `league_code` migration incidents, the immediate hardening slice is: verify no unresolved `_prisma_migrations` rows remain after deploy, verify the repaired migration end-state before calling `migrate resolve --applied`, detect the “marked applied but schema still wrong” case, and log the exact migrate task definition used by `migrate-qa`. |
| PDH-015 | Browser Deploy Gate | Harden the PoolMaster deploy-gate E2E lane so rollout and auth issues fail truthfully instead of flaking | In Progress | Current slice: remove CI retries, add a deployed frontend release probe before Playwright, keep the server as the source of truth for the CSRF cookie, and prove that unauthenticated invite preview remains public so `/invite/<code>` does not regress behind auth. |
