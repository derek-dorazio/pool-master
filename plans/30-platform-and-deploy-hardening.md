...
| PDH-012 | Runtime Drift | Remove unused Redis infrastructure and environment wiring from Terraform and service runtime | In Progress | Redis client usage does not appear in live app code. The cleanup slice removed Redis from CI test services, local docker compose, Terraform, OpenAPI export env setup, the admin health DTO/service/UI contract, and the old runtime dependency. Remaining work is to refresh generated artifacts so stale Redis fields disappear from generated OpenAPI/client output, and to clean the leftover docs/plan references that still describe Redis as active infrastructure. |
...
| PDH-009 | Node Actions | Upgrade GitHub Actions that still rely on Node 20 | In Progress | Audit confirmed the repo was still using `actions/checkout@v4`, `actions/setup-node@v4`, `aws-actions/configure-aws-credentials@v4`, `docker/build-push-action@v5`, and `docker/setup-buildx-action@v3` in active workflows. Local workflow updates now move CI/deploy to Node-24-safe major versions in both `.github/workflows/ci.yml` and `.github/workflows/deploy.yml`; remaining work is to exercise the next CI run and confirm there are no compatibility regressions. |
...
| PDH-011 | Coverage Gates | Enforce current coverage baselines locally and in GitHub CI, then ratchet upward over time | In Progress | Thresholds are already enforced locally and in CI through Jest/Vitest config. Remaining platform work is the CI visibility piece: upload coverage artifacts, print clearer suite totals to logs/summaries, and then ratchet further from a stable reported baseline. |
