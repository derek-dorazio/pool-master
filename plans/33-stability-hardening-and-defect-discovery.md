...
| SHD-004 | Coverage | Implement coverage threshold enforcement at the current measured baselines after collector cleanup | In Progress | Baseline thresholds are now enforced in Jest/Vitest and have been ratcheted twice as the fast suites improved. A follow-up CI regression fix calibrated the web thresholds to CI-stable floors while the merged worker slices raised the latest validated local baselines to 24.00 / 14.20 / 21.15 / 24.53 (backend), 57.33 / 48.65 / 56.79 / 60.24 (web), and 26.96 / 21.55 / 20.75 / 28.33 (admin). Remaining work is CI visibility and continued ratchets through targeted active-surface coverage. |
| SHD-005 | Test Cleanup | Audit lingering low-value compatibility/contract suites and remove or rewrite any that no longer add signal | In Progress | Initial assessment: the old DB-heavy integration and broad smoke suites are already gone. Current cleanup removes the mocked web pseudo-integration suites and the deferred draft-engine unit suites after explicit user approval; rerun the fast local gates and coverage baselines before marking this done. |
| SHD-006 | Defect Discovery | Increase coverage in backend Jest, web Vitest, admin Vitest, and DB-backed integration by targeting brittle or under-tested MVP areas | In Progress | The second worker wave stayed productive and clean: league onboarding/join flows now use real enums and honest retry behavior, admin platform/config surfaces have meaningful save/reset/loading/error coverage, backend league permissions enforce membership/commissioner boundaries, and the dashboard active-surface slice added contest aggregation, active-contest filtering, recent-activity limits, and a real sport-icon enum fix. The admin health/flags slice now also keeps the health dashboard aligned with the no-Redis architecture and synchronizes feature-flag detail state when async data arrives. The latest follow-up slices added commissioner invite-send coverage on league members, draft real-entry hook coverage, contest-create submit-contract coverage, and now a league create review/edit coverage pass, while the local DB-backed integration suite caught and confirmed the remaining compliance request-shape fix. Continue the same slice pattern into the remaining invite/member and contest-review seams before broadening slower suites. |
| SHD-007 | Anti-Patterns | Inventory and remove stale wrappers, legacy package usage, unsafe casts, and dead compatibility helpers that survived the earlier remediation | In Progress | The dead `clients/web/src/lib/api-client-generated.ts` and `clients/admin/src/lib/api-client-generated.ts` compatibility wrappers are removed. The temporary `getAccountDeletionStatus` helper in `clients/web/src/lib/api.ts` and the unused `ContestLookup.clear()` no-op in `stat-event-consumer.ts` are also removed. The latest admin health slice also removed the stale Redis visual assumption from the health dashboard, and the flags detail slice fixed an async state-sync bug. Remaining work is to continue scanning for narrower legacy shims and wrapper patterns in active MVP code paths. |
