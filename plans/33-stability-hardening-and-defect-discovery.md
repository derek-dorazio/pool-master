...
| SHD-007 | Anti-Patterns | Inventory and remove stale wrappers, legacy package usage, unsafe casts, and dead compatibility helpers that survived the earlier remediation | In Progress | The dead `clients/web/src/lib/api-client-generated.ts` and `clients/admin/src/lib/api-client-generated.ts` compatibility wrappers are removed. Remaining work is to review narrower shims like temporary SDK escape hatches and any no-op compatibility methods before deleting them. |
