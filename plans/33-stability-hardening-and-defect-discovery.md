...
|---|---|---|---|---|
| SHD-001 | Platform | Finish remaining Plan 30 priority items that directly improve deploy determinism and maintenance safety | In Progress | Remaining platform work is now concentrated in Plan 30 follow-through: GitHub Actions Node 20 upgrades, Terraform hygiene cleanup, coverage artifact visibility in CI, and final removal of stale mutable/runtime assumptions after the immutable ECS and frontend deploy work landed |
...
| SHD-006 | Defect Discovery | Increase coverage in backend Jest, web Vitest, admin Vitest, and DB-backed integration by targeting brittle or under-tested MVP areas | In Progress | The remaining execution work has now been split into [plans/35-active-coverage-execution.md](/Users/DDorazio/Library/CloudStorage/OneDrive-CURRICULUMASSOCIATESLLC/Documents/Claude/pool-master/plans/35-active-coverage-execution.md), with the current worker lanes focused on contest review/results, backend invite permissions, web settings/profile, admin audit/announcements, and backend dashboard/social edges. Continue treating faster suites as the primary defect-discovery path and add DB-backed integration only when the real backend boundary materially adds signal. |
