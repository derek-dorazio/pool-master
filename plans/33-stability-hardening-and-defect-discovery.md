...
| SHD-006 | Defect Discovery | Increase coverage in backend Jest, web Vitest, admin Vitest, and DB-backed integration by targeting brittle or under-tested MVP areas | In Progress | The auth pilot slice completed cleanly, and the first worker-safe slices exposed real defects instead of broad architectural trouble: scoring-consumer duplicate-entry dedupe, contest scoring-page state reset, admin notification payload alignment, and league member/settings SDK drift. Integrate, remeasure, and continue the same slice pattern before raising thresholds. |
