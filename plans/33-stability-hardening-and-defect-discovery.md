...
| SHD-006 | Defect Discovery | Increase coverage in backend Jest, web Vitest, admin Vitest, and DB-backed integration by targeting brittle or under-tested MVP areas | In Progress | The second worker wave stayed productive and clean: league onboarding/join flows now use real enums and honest retry behavior, admin platform/config surfaces have meaningful save/reset/loading/error coverage, and backend league permissions now enforce membership/commissioner boundaries for member-directory, audit, dashboard, and action-item reads. Continue the same slice pattern into league onboarding, admin platform, dashboard hooks, and permission/compliance edges before broadening slower suites. |
| SHD-007 | Anti-Patterns | Inventory and remove stale wrappers, legacy package usage, unsafe casts, and dead compatibility helpers that survived the earlier remediation | In Progress | The dead `clients/web/src/lib/api-client-generated.ts` and `clients/admin/src/lib/api-client-generated.ts` compatibility wrappers are removed. The temporary `getAccountDeletionStatus` helper in `clients/web/src/lib/api.ts` and the unused `ContestLookup.clear()` no-op in `stat-event-consumer.ts` are also removed. Remaining work is to continue scanning for narrower legacy shims and wrapper patterns in active MVP code paths. |
