...
|---|---|---|---|---|
| 37-001 | 1 | Finalize the top-level product boundary decision: leagues replace tenants as the primary customer/workspace boundary | Completed | Locked and implemented for active product flows on `codex-backend-refactor-lane`; product routes are league-first and no longer require request tenant context |
| 37-002 | 1 | Remove `Tenant` entirely from the target domain model and identify all schema, service, DTO, and admin surfaces that must be migrated off it | In Progress | Active runtime/admin tenant surfaces were removed or constrained; persistence/auth internals still retain `Tenant` compatibility and need a future follow-up lane for full elimination |
| 37-003 | 1 | Design the global `User` model with no `tenantId`, and confirm that all normal product access flows through `LeagueMembership` and `SquadMembership` | In Progress | Active product access now flows through league/squad membership, but `User.tenantId` remains as an internal compatibility field pending a later auth/identity cleanup |
...
| 37-004A | 1 | Redesign `LeagueMembership` around durable active/inactive lifecycle state with only `COMMISSIONER` and `MEMBER` roles | Completed | Implemented on `codex-backend-refactor-lane`: `LeagueMembership.status`, active-only member reads, inactive-on-remove/leave behavior, reactivation support, narrowed role route contract, and focused unit + DB integration coverage |
| 37-004B | 1 | Redesign `LeagueInvitation` as invite-only, single-use onboarding with generic codes and optional invited-email matching | Completed | Implemented on `codex-backend-refactor-lane`: accepted invites now create/reactivate `MEMBER` memberships, stay single-use, and are covered by invitation CRUD/integration tests |
| 37-004C | 1 | Add `Squad` and `SquadMembership` so contest-playing identity is separate from league membership and supports co-managers | Completed | Implemented on `codex-backend-refactor-lane` with schema, repositories, league-scoped squad APIs, one-squad-per-user-per-league enforcement, co-manager support, and inactive-on-last-manager removal behavior |
...
| 37-010 | 3 | Add `SportEventParticipant` as the canonical official event-field model with unique `(sportEventId, participantId)` membership | Completed | Implemented on `codex-backend-refactor-lane` with Prisma models, repositories, ingestion persistence, and repository integration coverage |
| 37-011 | 3 | Add `SportEventParticipantSourceData` and `SportEventParticipantValuation` to separate imported/provider data from PoolMaster-derived valuation data | Completed | Implemented on `codex-backend-refactor-lane` with raw + normalized source data persistence, valuation rows, repositories, and integration coverage |
| 37-012 | 3 | Redesign contest/event references so contests, pools, matchups, and picks use strong relations to global events and the full official event field | Completed | Active backend runtime now uses `Contest.sportEventId`, `RosterPick.sportEventParticipantId`, and removed old pool/matchup/pick prediction paths from first-pass scope |
...
| 37-013A | 3 | Redesign `ContestEntry` to reference `Squad` instead of direct league membership and support per-contest entry-count rules | Completed | Implemented on `codex-backend-refactor-lane` with `squadId`, `entryNumber`, `status`, `standingsPosition`, squad-aware create/list/delete flows, scoring/prize persistence, and focused unit + DB integration coverage |
