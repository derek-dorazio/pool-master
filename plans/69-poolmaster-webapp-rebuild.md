...
| In Progress | Build the first core product flows | The primitive commissioner/member experience is now partially aligned to the reviewed `/welcome`, `/league/<leagueCode>`, `/my-leagues`, and `/invite/<inviteCode>` model; create-league modal launch + submit is in place, league home and team home now expose team-context contest entry creation, contest detail now shows all entries with entry name + team name, and team-owned contest entries now follow the hybrid naming flow where backend default names are created immediately and owners can rename entries while the contest is still open. Richer commissioner/root-admin flows still remain. |
