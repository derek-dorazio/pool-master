...
| In Progress | Build layout primitives and role-aware navigation (69-C) | The shell is now moving onto the reviewed league-first model with a persistent header, league selector scaffold, logout, and placeholder league-scoped navigation; fuller header utilities and final visual design still remain |
| In Progress | Build the first product flow slice (69-D) | Auth now preserves deep-link intent, zero-league users route to `/welcome`, league pages are moving onto `/league/<leagueCode>`, invite entry is moving onto `/invite/<inviteCode>` with explicit acceptance, and commissioner invite-link generation now emits the new public route shape |
...
| In Progress | Build the first core product flows | The primitive commissioner/member experience is being realigned to the reviewed `/welcome`, `/league/<leagueCode>`, and `/invite/<inviteCode>` model; create-league wizard implementation, richer league home, and broader commissioner/root-admin flows still remain |
