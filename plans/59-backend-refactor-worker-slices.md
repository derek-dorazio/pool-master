...
| 59-006 | 3 | Implement Slice E: Contest core rebuild | In Progress | `ContestEntry` now uses `squadId`, `entryNumber`, `status`, and `standingsPosition`; `RosterPick` now references `sportEventParticipantId`; roster draft/pick flows resolve display state through `SportEventParticipant`; live standings reads plus commissioner recalculation now derive from `ContestEntry` instead of `ContestStanding`; periodic standings rollup no longer writes legacy `ContestStanding`; and completed contest history reads now fall back to `ContestEntry` + `ContestEntryPrizeAward` when legacy `ContestResult` / `PayoutHistory` snapshots are absent, with focused unit + DB integration coverage green for entry, roster selection, standings, recalculation, rollup, and fallback history reads |
