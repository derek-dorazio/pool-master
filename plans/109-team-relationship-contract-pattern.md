...
| 109-002 | 1 | Audit current Team DTOs, mappers, and permission hooks against the locked model | Not Started | Confirm exactly where `SquadDto`, Team lists, Team detail reads, and Team-derived entry flows need additive contract fields and where root-admin Team authority is still only implicit. |
| 109-003 | 2 | Add Team `teamRelationship` + `isRootAdmin` DTOs, mapper output, and OpenAPI/client refresh | Not Started | Required comments/descriptions must explicitly say `teamRelationship` is requester-relative relationship context, `isRootAdmin` is global platform state, and Team intentionally does not add a separate `memberType`/`ownerType` field in the first pass. |
| 109-004 | 2 | Patch backend Team authorization to make owner/commissioner/root-admin checks truthful | Not Started | Root-admin Team parity must become backend truth, not just frontend assumption. |
| 109-005 | 3 | Refactor frontend Team authority consumers to read backend-emitted contract truth | Not Started | Update `useTeamAuthority`, Team Home, Teams and Owners, and Team-derived entry surfaces to use `teamRelationship` plus `isRootAdmin` rather than local inference. |
| 109-006 | 3 | Decide and implement the Team-derived entry authority propagation pattern | Not Started | If Team-derived entry surfaces cannot truthfully compose from Team reads alone, add additive contract fields instead of frontend guesswork. |
| 109-007 | 4 | Add unit, contract, functional, and frontend verification for the Team authority pattern | Not Started | Must cover owner, commissioner, root-admin, and non-owner viewer cases, including truthful last-owner and inactive-Team behavior. |
| 109-008 | 4 | Reconcile Plan 107 references, Beads, and close Team authority drift | Not Started | Update any stale `useTeamAuthority` prose in Plan 107/108 so the docs match the shipped Team authority contract. |
