...
Quinn is commonly invoked as a subagent (isolated context). When invoked that
way, the invoker must pass the target scope (slice under verification, files
changed, risk profile) explicitly — the subagent starts with a fresh context
and cannot read prior conversation. Produce a structured findings report:
- what was run
- what passed
- what failed and why
- what was blocked
